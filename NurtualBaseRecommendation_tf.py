...
if __name__ == '__main__':
    ...
    # m1-100k
    csv_path_100k = os.getcwd() + '\\prepare_datasets\\m1-100k.csv'
    ...
    # 定义输入输出
    user_vertor = tf.placeholder(tf.float32, shape=(None,num_items), name='user_vector-input')
    item_vertor = tf.placeholder(tf.float32, shape=(None,num_users), name='item_vertor-input')
    y_ = tf.placeholder(tf.float32, shape=(None,1), name='y-input')
    # 定义深度神经网络前向传播过程
    layer1_user = tf.nn.relu(tf.matmul(user_vertor, W1_user))
    layer1_item = tf.nn.relu(tf.matmul(item_vertor, W1_item))
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            ...
            # sess.run(train_step, feed_dict={user_vertor: user_input[start:end], item_vertor: item_input[start:end],
            #                                 y_: labels[start:end]})
            sess.run(train_step, feed_dict={user_vertor: user_input[start:end], item_vertor: item_input[start:end],
                                           y_: labels[start:end]})
            if i % 100 == 0:
                ...
                total_cross_entropy = sess.run(cross_entropy, feed_dict={user_vertor: user_input, item_vertor: item_input,
                                           y_: labels})
                print(sess.run(W1_user))
                print(sess.run(biases1_user))
