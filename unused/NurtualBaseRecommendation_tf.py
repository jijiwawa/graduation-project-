...
if __name__ == '__main__':
    ...
    Tensor = tf.nn.softmax_cross_entropy_with_logits(logits=y, labels=y_)
    binary_cross_entropy = -tf.reduce_sum(Tensor)
    MAE = tf.metrics.mean_absolute_error(y_, y)
    # 论文提出的损失函数 nce
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(STEPS):
            ...
            accuracy_sroce = sess.run(accuracy, feed_dict={user_index: user_input[start:end],
                                                                       item_index: item_input[start:end],
                                                                       y_: labels[start:end]})
            # total,count  = sess.run(MAE,feed_dict={user_index:user_input_test,
            ...
            # print('mae_total',total)
            # print('mae_count',count)
