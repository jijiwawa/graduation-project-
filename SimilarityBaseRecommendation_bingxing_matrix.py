...
class SimilarityBaseRecommendation(Recommendation):
    ...
    def __init__(self, file, train_file, test_file):
        ...
        # 生成S2、S3矩阵middle
        self.matrix_S2 = self.Generate_S2_Matrix()
        self.matrix_S3 = self.Generate_S3_Matrix()
        self.matrix_middle = self.Generate_Middle_Matrix()
    ...
    def Generate_Middle_Matrix(self):
        matrixMiddle = np.ones((self.num_users, self.num_users), dtype=np.float32)
        for u in range(0, self.num_users):
            for v in range(0, self.num_users):
                matrixMiddle[u][v] = self.midfunciion(u, v)
        return matrixMiddle

    def Generate_S2_Matrix(self):
        matrix_S2 = np.ones((self.num_users, self.num_users), dtype=np.float32)
        for u in range(0, self.num_users):
            for v in range(0, self.num_users):
                matrix_S2[u][v] = self.S2(u, v)
        return matrix_S2

    def Generate_S3_Matrix(self):
        matrix_S3 = np.ones((self.num_users, self.num_users), dtype=np.float32)
        for u in range(0, self.num_users):
            for v in range(0, self.num_users):
                matrix_S3[u][v] = self.S3(u, v)
        return matrix_S3
    ...
    def Generate_UserSimilarity_Matrix(self):
        userSimilarityMatrix = self.matrix_S2 * self.matrix_S3 * self.matrix_middle
